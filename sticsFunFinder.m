            ...
            case 1 % 1 diffuser
                switch dType
                    case 'unconfined' % 1 unconfined diffuser in 2D
                        sticsFun=@(x,y,p)p(3)*exp((-(x(:,:,1)-p(4)).^2-(x(:,:,2)-p(5)).^2)/...
                            (2*y))./(2*pi*y);
                        msdFun=@(tau,p)2*p(1)*tau+p(2);
                        
                        if isempty(pstart)
                            pstart=[100,eps,100,0,0];
                        end
                        cpdLB=[0,-inf,0,-inf,-inf];
                        cpdUB=[inf,inf,inf,inf,inf];
                    case 'confined' % 1 confined diffuser in 2D
                        sticsFun=@(x,y,p)p(4)*exp((-x(:,:,1).^2-x(:,:,2).^2)/...
                            (2*y))./(2*pi*y);
                        msdFun=@(tau,p)abs(longmsd2d(p(1:3),tau));
                        
                        if isempty(pstart)
                            pstart=[10,10,eps,100];
                        end
                        cpdLB=[0,0,-inf,0];
                        cpdUB=[inf,inf,inf,inf];
                end
            case 2 % 2 diffusers
                switch dType
                    case 'unconfined' % 2 unconfined diffusers in 2D
                        sticsFun=@(x,y,p)p(3)*exp((-x(:,:,1).^2-x(:,:,2).^2)/2/y)./2/pi/y;
