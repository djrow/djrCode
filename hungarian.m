...
% ASSIGN assigned to each row and the minimum merit based on the assignment
% problem represented by the penalty, where the (i,j)th element represents 
% the merit to assign the jth job to the ith worker.
%
...
% un-assigned tasks. The merit returned only contains the merit of partially
% assigned tasks.
...
%{
[assignment,merit] = munkres(magic(5));
...
toc                 % about 2 seconds 
%}
% Example 3: rectangular assignment with inf merits
%{
A=rand(10,7);
A(A>0.7)=Inf;
[a,b]=munkres(A);
...
assignment = zeros(1,size(penalty,1));
merit = 0;

validMat = penalty == penalty & penalty < Inf;
bigM = 10^(ceil(log10(sum(penalty(validMat))))+1);
penalty(~validMat) = bigM;

% penalty(penalty~=penalty)=Inf;
% validMat = penalty<Inf;
...
maxv=10*max(penalty(validMat));

dMat = zeros(n) + maxv;
dMat(1:nRows,1:nCols) = penalty(validRow,validCol);
...
merit = -trace(penalty(assignment>0,assignment(assignment>0)));
