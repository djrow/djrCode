    ...
    for ii=1:numel(dnames)                % loop through movies
        ...
        elseif ~localFit
            ...
            for jj=find(cellfun(@(x)~isempty(x),tcorr)) % loop through cells in a movie
                ...
                %% fit correlations
                wSizeP=size(workingCorr);
                
                % space correlations for each tau must be in a cell element by themselves
                ind1=round(wSizeP/2-wSizeP(1)/4):round(wSizeP/2+wSizeP(1)/4);
                ind2=round(wSizeP/2-wSizeP(2)/4):round(wSizeP/2+wSizeP(2)/4);
                workingCorr=squeeze(mat2cell(workingCorr(ind1,ind2,2:end),...
                    numel(ind1),numel(ind1),ones(1,maxTau)))';
                
                % space domain
                [X,Y]=ndgrid((ind1-mean(ind1))*mpp,(ind2-mean(ind2))*mpp);
                X(~bphmask)=nan; Y(~bphmask)=nan;
                corrDom=cat(3,X,Y);
                corrDom={corrDom};
